import "axios";
import { EstateCondosApi } from "../../../../javascript/controllers/api/estate/estate_condos_api";
...
export default class extends ApplicationController {
  ...
  async submit(event) {
    ...
    let response = await EstateCondosApi.create({params: formData})
    this.handleResponse({response: response, resource: "estate_condos"})
  }
  ...
  formHTML() {
    return `
      <form class="contents" action="/estate_condos" accept-charset="UTF-8" method="post" data-${this.identifier}-target="form" data-action="submit->${this.identifier}#submit">
        ...
        <div class="inline">
          <input type="submit" name="commit" value="Create Estate condo" class="rounded-lg py-3 px-5 bg-blue-600 text-white inline-block font-medium cursor-pointer" data-disable-with="Create Estate condo">
        </div>
      </form>
    `
  }
