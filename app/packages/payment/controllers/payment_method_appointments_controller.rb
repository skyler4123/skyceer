class PaymentMethodAppointmentsController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @payment_method_appointment.update(payment_method_appointment_params)
        format.html { redirect_to @payment_method_appointment, notice: "Payment method appointment was successfully updated." }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to payment_method_appointments_path, status: :see_other, notice: "Payment method appointment was successfully destroyed." }
  ...
  private
    ...
    def payment_method_appointment_params
      params.expect(payment_method_appointment: [ :payment_owner_id, :payment_method_id ])
    end
end
