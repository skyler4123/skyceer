class CalendarUsersController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @calendar_user.save
        format.html { redirect_to @calendar_user, notice: "Calendar user was successfully created." }
  ...
  def update
    respond_to do |format|
      if @calendar_user.update(calendar_user_params)
        format.html { redirect_to @calendar_user, notice: "Calendar user was successfully updated." }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to calendar_users_path, status: :see_other, notice: "Calendar user was successfully destroyed." }
  ...
  private
    ...
    def set_calendar_user
      @calendar_user = CalendarUser.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def calendar_user_params
      params.expect(calendar_user: [ :calendar_userable_id, :calendar_userable_type, :discarded_at ])
    end
end
