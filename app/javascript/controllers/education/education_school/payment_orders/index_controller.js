...
export default class extends Education_EducationSchool_LayoutController {
  ...
  initTable() {
    let tableData = this.contentData().map((row) => {
      return {
        ...row,
        payment_customerable_name: row.payment_customerable ? row.payment_customerable.name : "",
        payment_userable_name: row.payment_userable ? row.payment_userable.name : "",
  ...
  defaultHTML() {
    return `
      <div id="education_rooms" class="w-full h-full"></div>
