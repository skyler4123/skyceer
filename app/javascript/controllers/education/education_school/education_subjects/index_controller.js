...
export default class extends Education_EducationSchool_LayoutController {
  ...
  initTable() {
    let tableData = this.contentData().map((row) => {
      return {
        ...row,
        name: `<a href="${CookieHelpers.navigationUrl()}/education_subjects/${row.id}">${row.name}</a>`,
        school_name: row.education_school ? row.education_school.name : "",
      }
    })
    var table = new Tabulator('#education_subjects', {
  ...
  defaultHTML() {
    return `
      <div id="education_subjects" class="w-full h-full"></div>
