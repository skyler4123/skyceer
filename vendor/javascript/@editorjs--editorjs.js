...
/**
   ...
   */static getNamespace(n){return n.split(".").reduce(((n,h)=>n&&Object.keys(n).length?n[h]:{}),le.currentDictionary)}};M.currentDictionary=I;let L=M;class Nt extends Error{}class Be{constructor(){this.subscribers={}}
/**
...
/**
   ...
   */on(n,h,p,g=!1){const m=Oo("l"),v={id:m,element:n,eventType:h,handler:p,options:g};if(!this.findOne(n,h,p))return this.allListeners.push(v),n.addEventListener(h,p,g),m
/**
...
/**
   ...
   */):null}expandToTag(n){const h=window.getSelection();h.removeAllRanges();const p=document.createRange();p.selectNodeContents(n),h.addRange(p)}}function Fo(n,h){const{type:p,target:g,addedNodes:m,removedNodes:v}=n;return(n.type!=="attributes"||n.attributeName!=="data-empty")&&!!(h.contains(g)||p==="childList"&&(Array.from(m).some((n=>n===h))||Array.from(v).some((n=>n===h))))}const O="redactor dom changed",N="block changed",P="fake cursor is about to be toggled",H="fake cursor have been set",z="editor mobile layout toggled";function Ft(n,h){if(!n.conversionConfig)return!1;const p=n.conversionConfig[h];return A(p)||Q(p)}function Le(n,h){return Ft(n.tool,h)}function Ht(n,h){return Object.entries(n).some((([n,p])=>h[n]&&Ao(h[n],p)))}async function zt(n,h){const p=(await n.save()).data;return h.reduce(((h,g)=>{if(!Ft(g,"import"))return h;const m=g.toolbox.filter((h=>{if(V(h)||!h.icon)return!1;if(h.data!==void 0){if(Ht(h.data,p))return!1}else if(g.name===n.name)return!1;return!0}));return h.push({...g,toolbox:m}),h}),[])}function vt(n,h){return!!n.mergeable&&(n.name===h.name||Le(h,"export")&&Le(n,"import"))}function Ho(n,h){const p=h==null?void 0:h.export;return A(p)?p(n):Q(p)?n[p]:(p!==void 0&&g("Conversion «export» property must be a string or function. String means key of saved data object to export. Function should export processed string to export."),"")}function wt(n,h){const p=h==null?void 0:h.import;return A(p)?p(n):Q(p)?{[p]:n}:(p!==void 0&&g("Conversion «import» property must be a string or function. String means key of tool data to import. Function accepts a imported string and return composed tool data."),{})}const U=class ke{
/**
...
/**
   ...
   */)}leafNodesAndReturnIndex(n){if(this.items.length===0)return this.cursor;let h=this.cursor;return h===-1?h=n===ke.directions.RIGHT?-1:0:this.items[h].classList.remove(this.focusedCssClass),h=n===ke.directions.RIGHT?(h+1)%this.items.length:(this.items.length+h-1)%this.items.length,d.canSetCaret(this.items[h])&&Oe((()=>b.setCursor(this.items[h])),50)(),this.items[h].classList.add(this.focusedCssClass),h}};U.directions={RIGHT:"right",LEFT:"left"};let $=U;class re{
/**
...
/**
   ...
   * @param renderParams - popover item render params
   */make(n,h){var p,g;const m=(h==null?void 0:h.wrapperTag)||"div",v=d.make(m,Ye.container,{type:m==="button"?"button":void 0});return n.name&&(v.dataset.itemName=n.name),this.nodes.icon=d.make("div",[Ye.icon,Ye.iconTool],{innerHTML:n.icon||oe}),v.appendChild(this.nodes.icon),n.title!==void 0&&v.appendChild(d.make("div",Ye.title,{innerHTML:n.title||""})),n.secondaryLabel&&v.appendChild(d.make("div",Ye.secondaryTitle,{textContent:n.secondaryLabel})),this.hasChildren&&v.appendChild(d.make("div",[Ye.icon,Ye.iconChevronRight],{innerHTML:X})),this.isActive&&v.classList.add(Ye.active),n.isDisabled&&v.classList.add(Ye.disabled),n.hint!==void 0&&((p=h==null?void 0:h.hint)==null?void 0:p.enabled)!==!1&&this.addHint(v,{...n.hint,position:((g=h==null?void 0:h.hint)==null?void 0:g.position)||"right"}),v
/**
...
/**
   ...
   */}handleItemClick(n){var h;n!==this.nestedPopoverTriggerItem&&((h=this.nestedPopoverTriggerItem)==null||h.handleClick(),super.destroyNestedPopoverIfExists()),super.handleItemClick(n)}}const _t=class ve{constructor(){this.scrollPosition=null}lock(){x?this.lockHard():document.body.classList.add(ve.CSS.scrollLocked)}unlock(){x?this.unlockHard():document.body.classList.remove(ve.CSS.scrollLocked)}lockHard(){this.scrollPosition=window.pageYOffset,document.documentElement.style.setProperty("--window-scroll-offset",`${this.scrollPosition}px`),document.body.classList.add(ve.CSS.scrollLockedHard)}unlockHard(){document.body.classList.remove(ve.CSS.scrollLockedHard),this.scrollPosition!==null&&window.scrollTo(0,this.scrollPosition),this.scrollPosition=null}};_t.CSS={scrollLocked:"ce-scroll-locked",scrollLockedHard:"ce-scroll-locked--hard"};let Lt=_t;const Ot=te("ce-popover-header"),Dt={root:Ot(),text:Ot("text"),backButton:Ot("back-button")};class ui{
/**
   * Constructs the instance
   *
...
/**
   ...
   * @param params - popover params
   */
constructor(n){super(n,{[Ge.Default]:{hint:{enabled:!1}},[Ge.Html]:{hint:{enabled:!1}}}),this.scrollLocker=new Lt,this.history=new pi,this.isHidden=!0,this.nodes.overlay=d.make("div",[it.overlay,it.overlayHidden]),this.nodes.popover.insertBefore(this.nodes.overlay,this.nodes.popover.firstChild),this.listeners.on(this.nodes.overlay,"click",(()=>{this.hide()})),this.history.push({items:n.items})}show(){this.nodes.overlay.classList.remove(it.overlayHidden),super.show(),this.scrollLocker.lock(),this.isHidden=!1}hide(){this.isHidden||(super.hide(),this.nodes.overlay.classList.add(it.overlayHidden),this.scrollLocker.unlock(),this.history.reset(),this.isHidden=!0)}destroy(){super.destroy(),this.scrollLocker.unlock()
/**
   * Handles displaying nested items for the item
   *
   * @param item – item to show nested popover for
...
/**
   ...
   */call(n,h){if(A(this.toolInstance[n])){n==="appendCallback"&&g("`appendCallback` hook is deprecated and will be removed in the next major release. Use `rendered` hook instead","warn");try{this.toolInstance[n].call(this.toolInstance,h)}catch(h){g(`Error during '${n}' call: ${h.message}`,"error")}}}
/**
...
/**
   ...
   *
   * @returns {object}
   */async save(){const n=await this.toolInstance.save(this.pluginsContent),h=this.unavailableTunesData;[...this.tunesInstances.entries(),...this.defaultTunesInstances.entries()].forEach((([n,p])=>{if(A(p.save))try{h[n]=p.save()}catch(n){g(`Tune ${p.constructor.name} save method throws an Error %o`,"warn",n)}}));const p=window.performance.now();let m;return Promise.resolve(n).then((n=>(m=window.performance.now(),{id:this.id,tool:this.name,data:n,tunes:h,time:m-p}))).catch((n=>{g(`Saving process for ${this.name} tool failed due to the ${n}`,"log","red")}))}
/**
...
/**
   ...
   */}async open(n=this.Editor.BlockManager.currentBlock){var h;this.opened=!0,this.selection.save(),this.Editor.BlockSelection.selectBlock(n),this.Editor.BlockSelection.clearCache();const{toolTunes:p,commonTunes:g}=n.getTunes();this.eventsDispatcher.emit(this.events.opened);const m=ue()?qt:St;this.popover=new m({searchable:!0,items:await this.getTunesItems(n,g,p),scopeElement:this.Editor.API.methods.ui.nodes.redactor,messages:{nothingFound:L.ui(Zt.ui.popover,"Nothing found"),search:L.ui(Zt.ui.popover,"Filter")}}),this.popover.on(Qe.Closed,this.onPopoverClose),(h=this.nodes.wrapper)==null||h.append(this.popover.getElement()),this.popover.show()}getElement(){return this.nodes.wrapper}
/**
   ...
   */async getTunesItems(n,h,p){const g=[];p!==void 0&&p.length>0&&(g.push(...p),g.push({type:Ge.Separator}));const m=Array.from(this.Editor.Tools.blockTools.values()),v=(await zt(n,m)).reduce(((h,p)=>(p.toolbox.forEach((g=>{h.push({icon:g.icon,title:L.t(Zt.toolNames,g.title),name:p.name,closeOnActivate:!0,onActivate:async()=>{const{BlockManager:h,Caret:m,Toolbar:v}=this.Editor,x=await h.convert(n,p.name,g.data);v.close(),m.setToBlock(x,m.positions.END)}})})),h)),[]);return v.length>0&&(g.push({icon:ge,name:"convert-to",title:L.ui(Zt.ui.popover,"Convert to"),children:{searchable:!0,items:v}}),g.push({type:Ge.Separator})),g.push(...h),g.map((n=>this.resolveTuneAliases(n)))
/**
...
/**
   ...
   */}composeBlockEvent(n,h){return new CustomEvent(n,{detail:h})}}const $o="block-removed",Yo="block-added",Vo="block-moved",Wo="block-changed";class Qi{constructor(){this.completed=Promise.resolve()}
/**
...
/**
   ...
   */}move(n,h=this.currentBlockIndex){isNaN(n)||isNaN(h)?g("Warning during 'move' call: incorrect indices provided.","warn"):this.validateIndex(n)&&this.validateIndex(h)?(this._blocks.move(n,h),this.currentBlockIndex=n,this.blockDidMutated(Vo,this.currentBlock,{fromIndex:h,toIndex:n})
/**
...
/**
   ...
   */}processDragOver(n){n.preventDefault()}}const Ko=180,qo=400;class rs extends y{
/**
   * Prepare the module
   *
   ...
   */
constructor({config:n,eventsDispatcher:h}){super({config:n,eventsDispatcher:h}),this.disabled=!1,this.batchingTimeout=null,this.batchingOnChangeQueue=new Map,this.batchTime=qo,this.mutationObserver=new MutationObserver((n=>{this.redactorChanged(n)})),this.eventsDispatcher.on(N,(n=>{this.particularBlockChanged(n.event)})),this.eventsDispatcher.on(P,(()=>{this.disable()})),this.eventsDispatcher.on(H,(()=>{this.enable()}))}enable(){this.mutationObserver.observe(this.Editor.UI.nodes.redactor,{childList:!0,subtree:!0,characterData:!0,attributes:!0}),this.disabled=!1}disable(){this.mutationObserver.disconnect(),this.disabled=!0
/**
...
/**
   ...
   */)}redactorChanged(n){this.eventsDispatcher.emit(O,{mutations:n})}}const Xo=class lo extends y{constructor(){super(...arguments),this.MIME_TYPE="application/x-editor-js",this.toolsTags={},this.tagsByTool={},this.toolsPatterns=[],this.toolsFiles={},this.exceptionList=[],this.processTool=n=>{try{const h=n.create({},{},!1);if(n.pasteConfig===!1){this.exceptionList.push(n.name);return}if(!A(h.onPaste))return;this.getTagsConfig(n),this.getFilesConfig(n),this.getPatternsConfig(n)}catch(h){g(`Paste handling for «${n.name}» Tool hasn't been set up because of the error`,"warn",h)}},this.handlePasteEvent=async n=>{const{BlockManager:h,Toolbar:p}=this.Editor,g=h.setCurrentBlockByChildNode(n.target);!g||this.isNativeBehaviour(n.target)&&!n.clipboardData.types.includes("Files")||g&&this.exceptionList.includes(g.name)||(n.preventDefault(),this.processDataTransfer(n.clipboardData),p.close())}}async prepare(){this.processTools()}
/**
   * Set read-only state
   *
...
/**
   ...
   * @returns {OutputData}
   */makeOutput(n){const h=[];return n.forEach((({id:n,tool:p,data:m,tunes:v,isValid:x})=>{if(!x){g(`Block «${p}» skipped because saved data is invalid`);return}if(p===this.Editor.Tools.stubTool){h.push(m);return}const w={id:n,type:p,data:m,...!V(v)&&{tunes:v}};h.push(w)})),{time:+new Date,blocks:h,version:"2.30.2"}}}(function(){try{if(typeof document<"u"){var n=document.createElement("style");n.appendChild(document.createTextNode(".ce-paragraph{line-height:1.6em;outline:none}.ce-block:only-of-type .ce-paragraph[data-placeholder-active]:empty:before,.ce-block:only-of-type .ce-paragraph[data-placeholder-active][data-empty=true]:before{content:attr(data-placeholder-active)}.ce-paragraph p:first-of-type{margin-top:0}.ce-paragraph p:last-of-type{margin-bottom:0}")),document.head.appendChild(n)}}catch(n){console.error("vite-plugin-css-injected-by-js",n)}})();const Go='<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" stroke-linecap="round" stroke-width="2" d="M8 9V7.2C8 7.08954 8.08954 7 8.2 7L12 7M16 9V7.2C16 7.08954 15.9105 7 15.8 7L12 7M12 7L12 17M12 17H10M12 17H14"/></svg>';function us(n){const h=document.createElement("div");h.innerHTML=n.trim();const p=document.createDocumentFragment();return p.append(...Array.from(h.childNodes)),p
/**
...
/**
   ...
   */static get toolbox(){return{icon:Go,title:"Text"}}}class ht{constructor(){this.commandName="bold"}
/**
   * Sanitizer Rule
   * Leave <b> tags
   *
...
/**
   * @param api - Editor.js API
   */
constructor({api:n}){this.i18nAPI=n.i18n,this.blocksAPI=n.blocks,this.selectionAPI=n.selection,this.toolsAPI=n.tools,this.caretAPI=n.caret}async render(){const n=b.get(),h=this.blocksAPI.getBlockByElement(n.anchorNode);if(h===void 0)return[];const p=this.toolsAPI.getBlockTools(),g=await zt(h,p);if(g.length===0)return[];const m=g.reduce(((n,p)=>{var g;return(g=p.toolbox)==null||g.forEach((g=>{n.push({icon:g.icon,title:L.t(Zt.toolNames,g.title),name:p.name,closeOnActivate:!0,onActivate:async()=>{const n=await this.blocksAPI.convert(h.id,p.name,g.data);this.caretAPI.setToBlock(n,"end")}})})),n}),[]),v=await h.getActiveToolboxEntry(),x=v!==void 0?v.icon:ge,w=!ue();return{icon:x,name:"convert-to",hint:{title:this.i18nAPI.t("Convert to")},children:{searchable:w,items:m,onOpen:()=>{w&&(this.selectionAPI.setFakeBackground(),this.selectionAPI.save())},onClose:()=>{w&&(this.selectionAPI.restore(),this.selectionAPI.removeFakeBackground())}}}}}ao.isInline=!0;class co{
/**
...
/**
   ...
   *
   * @returns {HTMLElement}
   */make(){const n=d.make("div",this.CSS.wrapper),h=be,p=d.make("div",this.CSS.info),g=d.make("div",this.CSS.title,{textContent:this.title}),m=d.make("div",this.CSS.subtitle,{textContent:this.subtitle});return n.innerHTML=h,p.appendChild(g),p.appendChild(m),n.appendChild(p),n}}co.isReadOnlySupported=!0;class ps extends ct{constructor(){super(...arguments),this.type=Do.Inline}get title(){return this.constructable[Uo.Title]}create(){return new this.constructable({api:this.api,config:this.settings})}}class fs extends ct{constructor(){super(...arguments),this.type=Do.Tune
/**
   * Constructs new BlockTune instance from constructable
   *
   * @param data - Tune data
   * @param block - Block API object
   */}create(n,h){return new this.constructable({api:this.api,config:this.settings,block:h,data:n})}}class F extends Map{get blockTools(){const n=Array.from(this.entries()).filter((([,n])=>n.isBlock()));return new F(n)}get inlineTools(){const n=Array.from(this.entries()).filter((([,n])=>n.isInline()));return new F(n)}get blockTunes(){const n=Array.from(this.entries()).filter((([,n])=>n.isTune()));return new F(n)}get internalTools(){const n=Array.from(this.entries()).filter((([,n])=>n.isInternal));return new F(n)}get externalTools(){const n=Array.from(this.entries()).filter((([,n])=>!n.isInternal));return new F(n)}}var Qo=Object.defineProperty,Jo=Object.getOwnPropertyDescriptor,ho=(n,h,p,g)=>{for(var m,v=g>1?void 0:g?Jo(h,p):h,x=n.length-1;x>=0;x--)(m=n[x])&&(v=(g?m(h,p,v):m(v))||v);return g&&v&&Qo(h,p,v),v};class ft extends ct{constructor(){super(...arguments),this.type=Do.Block,this.inlineTools=new F,this.tunes=new F
/**
...
/**
   ...
   */getConstructor(n){switch(!0){case n[Uo.IsInline]:return ps;case n[jo.IsTune]:return fs;default:return ft}}}class uo{
/**
...
/**
   ...
   */processBottomZoneClick(n){const h=this.Editor.BlockManager.getBlockByIndex(-1),p=d.offset(h.holder).bottom,g=n.pageY,{BlockSelection:m}=this.Editor;if(n.target instanceof Element&&n.target.isEqualNode(this.nodes.redactor)&&!m.anyBlockSelected&&p<g){n.stopImmediatePropagation(),n.stopPropagation();const{BlockManager:h,Caret:p,Toolbar:g}=this.Editor;(!h.lastBlock.tool.isDefault||!h.lastBlock.isEmpty)&&h.insertAtEnd(),p.setToTheLastBlock(),g.moveAndOpen(h.lastBlock)}}selectionChanged(){const{CrossBlockSelection:n,BlockSelection:h}=this.Editor,p=b.anchorElement;n.isCrossBlockSelectionStarted&&h.anyBlockSelected&&b.get().removeAllRanges(),p?p.closest(`.${D.CSS.content}`)===null&&(this.Editor.InlineToolbar.containsNode(p)||this.Editor.InlineToolbar.close(),!(p.dataset.inlineToolbar==="true"))||(this.Editor.BlockManager.currentBlock||this.Editor.BlockManager.setCurrentBlockByChildNode(p),this.Editor.InlineToolbar.tryToShow(!0)):b.range||this.Editor.InlineToolbar.close()}enableInputsEmptyMark(){function e(n){const h=n.target;At(h)}this.readOnlyMutableListeners.on(this.nodes.wrapper,"input",e),this.readOnlyMutableListeners.on(this.nodes.wrapper,"focusin",e),this.readOnlyMutableListeners.on(this.nodes.wrapper,"focusout",e)}}const ai={BlocksAPI:fi,CaretAPI:mi,EventsAPI:bi,I18nAPI:st,API:ki,InlineToolbarAPI:vi,ListenersAPI:wi,NotifierAPI:Bi,ReadOnlyAPI:Ti,SanitizerAPI:Li,SaverAPI:_i,SelectionAPI:Ni,ToolsAPI:Pi,StylesAPI:Di,ToolbarAPI:Ri,TooltipAPI:Fi,UiAPI:Hi,BlockSettings:Ui,Toolbar:qi,InlineToolbar:Zi,BlockEvents:Gi,BlockManager:es,BlockSelection:ts,Caret:De,CrossBlockSelection:os,DragNDrop:is,ModificationsObserver:rs,Paste:Zo,ReadOnly:as,RectangleSelection:we,Renderer:cs,Saver:ds,Tools:go,UI:ys};class Bs{
/**
...
/**
   ...
   */}set configuration(h){var p,g;R(h)?this.config={...h}:this.config={holder:h},Ze(!!this.config.holderId,"config.holderId","config.holder"),this.config.holderId&&!this.config.holder&&(this.config.holder=this.config.holderId,this.config.holderId=null),this.config.holder==null&&(this.config.holder="editorjs"),this.config.logLevel||(this.config.logLevel=n.VERBOSE),xo(this.config.logLevel),Ze(!!this.config.initialBlock,"config.initialBlock","config.defaultBlock"),this.config.defaultBlock=this.config.defaultBlock||this.config.initialBlock||"paragraph",this.config.minHeight=this.config.minHeight!==void 0?this.config.minHeight:300;const m={type:this.config.defaultBlock,data:{}};this.config.placeholder=this.config.placeholder||!1,this.config.sanitizer=this.config.sanitizer||{p:!0,b:!0,a:!0},this.config.hideToolbar=!!this.config.hideToolbar&&this.config.hideToolbar,this.config.tools=this.config.tools||{},this.config.i18n=this.config.i18n||{},this.config.data=this.config.data||{blocks:[]},this.config.onReady=this.config.onReady||(()=>{}),this.config.onChange=this.config.onChange||(()=>{}),this.config.inlineToolbar=this.config.inlineToolbar===void 0||this.config.inlineToolbar,(V(this.config.data)||!this.config.data.blocks||this.config.data.blocks.length===0)&&(this.config.data={blocks:[m]}),this.config.readOnly=this.config.readOnly||!1,(p=this.config.i18n)!=null&&p.messages&&L.setDictionary(this.config.i18n.messages),this.config.i18n.direction=((g=this.config.i18n)==null?void 0:g.direction)||"ltr"
/**
 ...
 */class Ts{static get version(){return"2.30.2"}
/**
   * @param {EditorConfig|string|undefined} [configuration] - user configuration
   */constructor(n){let t=()=>{};R(n)&&A(n.onReady)&&(t=n.onReady);const h=new Bs(n);this.isReady=h.isReady.then((()=>{this.exportAPI(h),t()}))}
/**
